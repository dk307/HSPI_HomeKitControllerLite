...
namespace System.Net.Http
{
    internal static class HttpRuleParser
    {
        ...
        [Diagnostics.CodeAnalysis.SuppressMessage("Major Code Smell", "S1066:Collapsible \"if\" statements should be merged", Justification = "<Pending>")]
            ...
            char closeChar, bool supportsNesting, ref int nestedCount, out int length)
        {
            ...
            while (current < input.Length)
            {
                ...
                if ((current + 2 < input.Length) &&
                    (GetQuotedPairLength(input, current, out int quotedPairLength) == HttpParseResult.Parsed))
                {
                ...
                if (supportsNesting && (input[current] == openChar))
                {
                    ...
                    try
                    {
                        ...
                        HttpParseResult nestedResult = GetExpressionLength(input, current, openChar, closeChar,
                            supportsNesting, ref nestedCount, out int nestedLength);
